...
+ lib: library for core parser and collection 
  - gspcore32.lib, 32-bit version core parser library for windows
  - libgspcore32.a, 32-bit version core parser library for Linux
  - gspcollection32.lib, 32-bit version extension library for windows
	+ you can build this library from the source code under ext/collection
  - libgspcollection32.a, 32-bit version core parser library for Linux
    + you can build this library from the source code under ext/collection
...
then, generate first.exe by running:
    ...
    cl first.c  /I ../core /link ../lib/gspcore32.lib


<h4>compile this simple C program under Linux using gcc</h4>

    gcc -O2 first.c -I../core -L../lib -lgspcore32 -o first
...
+ Jan 2013
	...
	- libray shipped in 32-bit for windows and linux, 64-bit library is avaiable on request(info@sqlparser.com) 
	- demos demostrate how to 
...
typedef enum EStmtType {
		...
		sstmssqlalterauthorization,sstmssqlaltercertificate,sstmssqlaltercredential,sstmssqlalterdatabase,sstmssqlalterendpoint,
		...
		sstmssqldrop,sstmssqlstmtstub,sstmssqlcheckpoint,sstoraclealtercluster,sstoraclealterdatabase,
		sstmssqlexecfake,
...
typedef struct gsp_lexer{
	...
	char	*yyinput; /* input buffer */
	char	*yyline;	//current input line
	int		yylineno,yycolno;	//current input position
	int		yylineno_p;
	int		yycolno_p;
	char	*yytext;	//matched text (should be considered r/o)
	int		yyleng;		//length of matched text		

	char	*linePtr;		//pointer to current active char in line
	...
	char* (*getTokenText)(struct gsp_lexer *);
	...
	*/
	...
	char *dolqstart;//postgresql, start part of Dollar-quoted String Constants
	char *dolqbuf;
	size_t dolqbufsize;
	size_t dolqlen;
	...
	char	yyactchar;	//current character
	char	yylastchar;	//last matched character (0 if none)
...
typedef struct gsp_yyparser{
	...
	int *nUsed; // a list of number related to pMemp[n], means used memory
	int nMem; // number of memory slots current used
	int nMemAllocated; // number of memory slots current allocated
...
int		gsp_string_indexOf(const char *str, int len,const char *substr);//removed const twice
